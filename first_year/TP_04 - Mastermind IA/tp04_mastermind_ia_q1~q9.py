...
def choixCombinaison(Ci, Ri):
    """ demande à l'utilisateur/IA une combinaison. renvoie cette combinaison """
    if ipt == 'H':
        Cj = [NC]*LC
        for i in range(LC):
        ...
        while jouer != True:
            jouer = True
            for i in range(LC):
                Cj[i] = rd.randint(0, NC-1)
            for i in range(len(Ci)):
                jouer = jouer and (calculReponse(Cj, Ci[i]) == Ri[i])
        return Cj
